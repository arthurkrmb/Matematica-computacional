# Colab: https://colab.research.google.com/drive/1cIlxt36gCs6JuFOlYNuGxSlyBlolsbdJ

import numpy as np
import matplotlib.pyplot as plt
xs = [1, 0]
ys = [0, 1]
def multiplicarvetorpormatriz(xs,ys,matriz):
    txv1 = (xs[0]*matriz[0] + ys[0]*matriz[1] )
    tyv1 = (xs[0]*matriz[2] + ys[0]*matriz[3] )
    txv2 = (xs[1]*matriz[0] + ys[1]*matriz[1] )
    tyv2 = (xs[1]*matriz[2] + ys[1]*matriz[3] )
    xs = [txv1, txv2]
    ys = [tyv1, tyv2]
    return xs,ys
...
matriz = [int(x) for x in input("Digite os valores da matriz separados por vírgula: ").split(',')]

xs, ys = mudarvetoresnografico(xs, ys)
xs, ys = multiplicarvetorpormatriz(xs,ys,matriz)
